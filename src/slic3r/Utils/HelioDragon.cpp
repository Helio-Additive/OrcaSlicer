...
void HelioQuery::request_remaining_optimizations(const std::string&            helio_api_url,
            ...
            try {
                if (parsed_obj.contains("data") && parsed_obj["data"].contains("printers")) {
                    ...
                    if (materials.contains("pageInfo") && materials["pageInfo"].contains("hasNextPage") &&
                        ...
                        Slic3r::GUI::wxGetApp().plater_->set_printers_from_helio();
                    }
                }
            } catch (...) {}
        })
            ...
            try {
                if (parsed_obj.contains("data") && parsed_obj["data"].contains("materials")) {
                    ...
                    if (materials.contains("pageInfo") && materials["pageInfo"].contains("hasNextPage") &&
                        ...
                        Slic3r::GUI::wxGetApp().plater_->set_materials_from_helio();
                    }
                }
            } catch (...) {}
        })
...
void HelioQuery::optimization_feedback(
    const std::string helio_api_url, const std::string helio_api_key, std::string optimization_id, float rating, std::string comment)
{
    std::string query_body = R"({
    ...
    query_body = boost::str(boost::format(query_body) % optimization_id % rating % comment);
    ...
    http.timeout_connect(20)
        .timeout_max(100)
        .on_complete([=](std::string body, unsigned status) { BOOST_LOG_TRIVIAL(info) << "optimization_feedback" << body; })
        .on_error([](std::string body, std::string error, unsigned status) {})
        .perform();
}
                ...
                while (res.status_str != "READY" && res.status_str != "ERROR" && poll_count < 60) {
                    ...
                    if (poll_res.success) {
                        ...
                        if (res.status_str == "ERROR") {
                            res.success = false;
                            res.error   = "Something went wrong while creating GCode. Please check your custom GCodes";
                        }
                                                          ...
                                                          float              stabilizedAirTemperature)
{
    ...
    if (temperatureStabilizationHeight != -1) {
        settings_fields.push_back(
            boost::str(boost::format(R"(                    "temperatureStabilizationHeight": %1%)") % temperatureStabilizationHeight));
    }

    if (airTemperatureAboveBuildPlate != -1) {
        settings_fields.push_back(
            boost::str(boost::format(R"(                    "airTemperatureAboveBuildPlate": %1%)") % airTemperatureAboveBuildPlate));
    }

    if (stabilizedAirTemperature != -1) {
        settings_fields.push_back(
            boost::str(boost::format(R"(                    "stabilizedAirTemperature": %1%)") % stabilizedAirTemperature));
    }

    std::string settings_block;
    if (!settings_fields.empty()) {
        settings_block = boost::join(settings_fields, ",\n");
    }
                                                            ...
                                                            int                layersToOptimizeEnd)
{
    ...
    optimization_fields.push_back(
        boost::str(boost::format(R"("layersToOptimize": [{"fromLayer": %1%, "toLayer": %2%}])") % actualStartLayer % layersToOptimizeEnd));
...
std::string HelioQuery::create_optimization_default_get(const std::string helio_api_url,
                                                        const std::string helio_api_key,
                                                        const std::string gcode_id)
{
    std::string res;
    ...
    http.timeout_connect(20)
        .timeout_max(100)
        .on_header_callback([&response_headers](std::string headers) { response_headers += headers; })
        .on_complete([&res](std::string body, unsigned status) { nlohmann::json parsed_obj = nlohmann::json::parse(body); })
...
void HelioQuery::stop_simulation(const std::string helio_api_url, const std::string helio_api_key, const std::string simulation_id)
{
    ...
    http.timeout_connect(20)
        ...
        .on_complete(
            [=](std::string body, unsigned status) { BOOST_LOG_TRIVIAL(info) << (boost::format("stop_simulation: success")).str(); })
        .on_error([=](std::string body, std::string error, unsigned status) {
                                                                             ...
                                                                             OptimizationInput oinput)
{
    ...
    if (!oinput.isDefault()) {
        ...
        query_body = generate_optimization_graphql_query(gcode_id, outer_wall, layer_threshold_meters, initial_room_temp_kelvin,
                                                         object_proximity_airtemp_kelvin, min_velocity, max_velocity, min_volumetric_speed,
                                                         max_volumetric_speed, oinput.layers_to_optimize[0], oinput.layers_to_optimize[1]);
    } else {
        query_body = generate_optimization_graphql_query(gcode_id, outer_wall, layer_threshold_meters, initial_room_temp_kelvin,
                                                         object_proximity_airtemp_kelvin, oinput.min_velocity, oinput.max_velocity,
                                                         oinput.min_volumetric_speed, oinput.max_volumetric_speed,
                                                         oinput.layers_to_optimize[0], oinput.layers_to_optimize[1]);
    }
...
void HelioQuery::stop_optimization(const std::string helio_api_url, const std::string helio_api_key, const std::string optimization_id)
{
    ...
    http.timeout_connect(20)
        ...
        .on_complete(
            [=](std::string body, unsigned status) { BOOST_LOG_TRIVIAL(info) << (boost::format("stop_optimization: success")).str(); })
        .on_error([=](std::string body, std::string error, unsigned status) {
...
void HelioBackgroundProcess::create_optimization_step(HelioQuery::CreateGCodeResult              create_gcode_res,
                                                      std::unique_ptr<GUI::NotificationManager>& notification_manager)
{
    if (create_gcode_res.success && !was_canceled()) {
        Slic3r::PrintBase::SlicingStatus status = Slic3r::PrintBase::SlicingStatus(15, _u8L("Helio: GCode created successfully"));
        ...
        const std::string                    gcode_id                = create_gcode_res.id;
        HelioQuery::CreateOptimizationResult create_optimization_res = HelioQuery::create_optimization(helio_api_url, helio_api_key,
                                                                                                       gcode_id, optimization_input_data);
...
void HelioBackgroundProcess::load_helio_file_to_viwer(std::string file_path, std::string tmp_path, HelioQuery::RatingData rating_data)
{
    ...
    Slic3r::HelioCompletionEvent* evt = new Slic3r::HelioCompletionEvent(GUI::EVT_HELIO_PROCESSING_COMPLETED, 0, file_path, tmp_path, true,
                                                                         "", rating_data.action, rating_data.qualityMeanImprovement,
                                                                         rating_data.qualityStdImprovement);
    wxQueueEvent(GUI::wxGetApp().plater(), evt);
}
