...
public:
    ...
    virtual wxString CheckValid(const wxString& value) const
    {
        return wxEmptyString;
    }; // return wxEmptyString if valid, otherwise return error message
...
{
public:
    TextInputValDoubleRangeChecker(double min_val, double max_val, bool enable_empty = false) : m_min_value(min_val), m_max_value(max_val)
    {
        EnableEmpty(enable_empty);
    };
    virtual wxString CheckValid(const wxString& value) const override;
