...
private:
    wxBoxSizer* create_input_item(
        wxWindow* parent, std::string key, wxString name, wxString unit, const std::vector<std::shared_ptr<TextInputValChecker>>& checkers);
    wxBoxSizer* create_combo_item(
        wxWindow* parent, std::string key, wxString name, std::map<int, wxString> combolist, int def, int width = 120);
...
public:
    HelioRatingDialog(
        wxWindow* parent = nullptr, int original = 0, int optimized = 0, std::string mean_impro = "", std::string std_impro = "");
    ~HelioRatingDialog() {};
