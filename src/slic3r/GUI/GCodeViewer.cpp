...
// Helio: Thermal index visualization
            case libvgcode::EViewType::ThermalIndexMean:
                if (is_extrusion && vertex.thermal_index_mean > -100.0f)
                    sprintf(detail_buf, "%s%.1f", _u8L("TI Mean: ").c_str(), vertex.thermal_index_mean);
                else
                    sprintf(detail_buf, "%s%s", _u8L("TI Mean: ").c_str(), "null");
                break;
            case libvgcode::EViewType::ThermalIndexMin:
                if (is_extrusion && vertex.thermal_index_min > -100.0f)
                    sprintf(detail_buf, "%s%.1f", _u8L("TI Min: ").c_str(), vertex.thermal_index_min);
                else
                    sprintf(detail_buf, "%s%s", _u8L("TI Min: ").c_str(), "null");
                break;
...
// Helio: Show TI values in tooltip when in a TI view
            if (view_type == libvgcode::EViewType::ThermalIndexMean ||
                ...
                add_row(_u8L("TI Min"), ti_str(vertex.thermal_index_min));
                add_row(_u8L("TI Mean"), ti_str(vertex.thermal_index_mean));
                add_row(_u8L("TI Max"), ti_str(vertex.thermal_index_max));
            }
