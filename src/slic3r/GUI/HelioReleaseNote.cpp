...
// #include "slic3r/GUI/GCodeRenderer/BaseRenderer.hpp"  // BambuStudio-specific, not in OrcaSlicer
#include <wx/regex.h>
...
void HelioStatementDialog::report_consent_install()
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " consent: " << post_body_str;
}
...
void HelioStatementDialog::create_pat_page()
{
    ...
    copy_pat_button->SetToolTip(_L("Personal Access Token (PAT)\n\nA secure credential that verifies your identity with Helio services. "
        "It's automatically used by OrcaSlicer for optimizations and simulations.\n\n"
...
{
    // Set Helio icon (not OrcaSlicer icon)
    wxBitmap bmp = create_scaled_bitmap("helio_icon", this, 32);
    wxIcon icon;
    icon.CopyFromBitmap(bmp);
    SetIcon(icon);
    ...
    bool helio_enabled = wxGetApp().app_config->get_bool("enable_helio_processing");
    if (!helio_enabled) {
    ...
    auto info_icon = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("info", this, 14), wxDefaultPosition, wxSize(FromDIP(14), FromDIP(14)), 0);
...
void ExpandCenterDialog::report_consent_unstall()
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << "  report_consent:" << post_body_str;
}
