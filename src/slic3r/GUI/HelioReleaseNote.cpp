...
HelioRemainUsageTime::HelioRemainUsageTime(wxWindow* parent, wxString label) : wxPanel(parent) { Create(label); }
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    Label* text = new Label(this, Label::Body_14,
                            _L("Failed to obtain Helio PAT. The number of issued PATs has reached the upper limit. Please pay attention to "
                               "the information on the Helio official website. Click Refresh to get it again once it is available."),
                            LB_AUTO_WRAP);
    ...
    auto helio_wiki_link = new LinkLabel(this, _L("Click for more details"),
                                         wxGetApp().app_config->get("language") == "zh_CN" ? "https://wiki.helioadditive.com/zh/home" :
                                                                                             "https://wiki.helioadditive.com/en/home");
...
Slic3r::HelioQuery::OptimizationInput HelioInputDialog::get_optimization_input(bool& ok)
{
    ...
    if (it != m_input_items.end()) {
        if (!m_input_items["chamber_temp_for_optimization"]->GetTextCtrl()->GetValue().IsEmpty() &&
            !s_get_double_val(m_input_items["chamber_temp_for_optimization"], data.chamber_temp)) {
            return data;
        }
    }
    ...
    if (!s_get_int_val(m_input_items["layers_to_optimize_min"], data.layers_to_optimize[0])) {
        return data;
    }
    if (!s_get_int_val(m_input_items["layers_to_optimize_max"], data.layers_to_optimize[1])) {
        return data;
    }
    ...
    if (limits_selection == LIMITS_SLICER_DEFAULT) {
        ...
        if (!s_get_double_val(m_input_items["min_velocity"], data.min_velocity)) {
            return data;
        }
        if (!s_get_double_val(m_input_items["max_velocity"], data.max_velocity)) {
            return data;
        }
        if (!s_get_double_val(m_input_items["min_volumetric_speed"], data.min_volumetric_speed)) {
            return data;
        }
        if (!s_get_double_val(m_input_items["max_volumetric_speed"], data.max_volumetric_speed)) {
            return data;
        }
    }
...
wxBoxSizer* HelioInputDialog::create_input_item(
    wxWindow* parent, std::string key, wxString name, wxString unit, const std::vector<std::shared_ptr<TextInputValChecker>>& checkers)
{
    ...
    TextInput* m_input_item = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition,

                                            wxSize(FromDIP(120), -1), wxTE_PROCESS_ENTER, wxEmptyString, unit);
    ...
    StateColor input_bg(std::pair<wxColour, int>(wxColour("#F0F0F1"), StateColor::Disabled),
                        std::pair<wxColour, int>(parent_bg, StateColor::Enabled));
...
wxBoxSizer* HelioInputDialog::create_combo_item(
    wxWindow* parent, std::string key, wxString name, std::map<int, wxString> combolist, int def, int width)
{
    wxBoxSizer* item_sizer  = new wxBoxSizer(wxHORIZONTAL);
    Label*      inout_title = new Label(parent, Label::Body_14, name);
    inout_title->SetFont(::Label::Body_14);
    ...
    auto combobox = new ::ComboBox(parent, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(width), -1), 0, nullptr,
                                   wxCB_READONLY);
...
wxBoxSizer* HelioInputDialog::create_input_optimize_layers(wxWindow* parent, int layer_count)
{
    ...
    TextInput* m_layer_min_item = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition,
                                                wxSize(FromDIP(76), -1), wxTE_PROCESS_ENTER);
    ...
    TextInput* m_layer_max_item = new TextInput(parent, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition,
                                                wxSize(FromDIP(76), -1), wxTE_PROCESS_ENTER);
...
void HelioInputDialog::update_action(int action)
{
    ...
    if (action == 0) {
        ...
        helio_wiki_link->setLinkUrl(wxGetApp().app_config->get("language") == "zh_CN" ? "https://wiki.helioadditive.com/zh/home" :
                                                                                        "https://wiki.helioadditive.com/en/home");
                ...
                CallAfter([=]() {
                    if (times > 0 || addons > 0) {
                        ...
                        if (times <= 0) {
                            ...
                            if (m_remain_usage_time) {
                                m_remain_usage_time->UpdateHelpTips(0);
                            }
                        } else {
                            buy_now_link->setLabel(_L("Buy add-ons"));
                            if (m_remain_usage_time) {
                                m_remain_usage_time->UpdateHelpTips(1);
                            }
                        }
                    }
                    Layout();
                    ...
                    if (m_remain_usage_time) {
                        m_remain_usage_time->UpdateRemainTime(times);
                    }
                    if (m_remain_purchased_time) {
                        m_remain_purchased_time->UpdateRemainTime(addons);
                    }
                });
            } else {
        ...
        buy_now_link->Show();
        helio_wiki_link->setLinkUrl(wxGetApp().app_config->get("language") == "zh_CN" ?
                                        "https://wiki.helioadditive.com/zh/optimization/quick-start" :
                                        "https://wiki.helioadditive.com/en/optimization/quick-start");
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    is_no_chamber               = !DeviceManager::get_value_from_config<bool>(source_model, "print", "support_chamber");
    ...
    wxBoxSizer* chamber_temp_item_for_simulation = create_input_item(panel_simulation, "chamber_temp_for_simulation",
                                                                     is_no_chamber ? _L("Environment Temperature") :
                                                                                     _L("Chamber Temperature"),
                                                                     wxT("\u00B0C"), {chamber_temp_checker});
    ...
    Label* sub_simulation = new Label(
        panel_simulation,
        _L("Note: Adjust the above temperature based on actual conditions. More accurate data ensures more precise analysis results."));
    ...
    wxStaticBitmap* panel_pay_loading_icon = new wxStaticBitmap(panel_pay_optimization, wxID_ANY,
                                                                create_scaled_bitmap("helio_loading", panel_optimization, 109),
                                                                wxDefaultPosition, wxSize(FromDIP(136), FromDIP(109)));

    Label* pay_sub = new Label(panel_pay_optimization,
                               _L("Helio's advanced feature for power users: available post purchase on its official website."));
    ...
    if (is_no_chamber) {
        chamber_temp_item_for_optimization = create_input_item(panel_optimization, "chamber_temp_for_optimization",
                                                               is_no_chamber ? _L("Environment Temperature") : _L("Chamber Temperature"),
                                                               wxT("\u00B0C"), {chamber_temp_checker});
        m_input_items["chamber_temp_for_optimization"]->GetTextCtrl()->SetHint(wxT("5-70"));

        sub_optimization = new Label(
            panel_optimization,
            _L("Note: Adjust the above temperature based on actual conditions. More accurate data ensures more precise analysis results."));
    ...
    float max_speed = 0.0;
    if (plater) {
        plater->get_preview_min_max_value_of_option((int) Slic3r::GUI::GCodeViewer::EViewType::Feedrate, min_speed, max_speed);
    }
    wxBoxSizer* min_velocity_item = create_input_item(panel_velocity_volumetric, "min_velocity", _L("Min Velocity"), wxT("mm/s"),
                                                      {double_min_checker});
    m_input_items["min_velocity"]->GetTextCtrl()->SetLabel(wxString::Format("%.0f", s_round(min_speed, 0)));

    wxBoxSizer* max_velocity_item = create_input_item(panel_velocity_volumetric, "max_velocity", _L("Max Velocity"), wxT("mm/s"),
                                                      {double_min_checker});
    ...
    if (plater) {
        plater->get_preview_min_max_value_of_option((int) Slic3r::GUI::GCodeViewer::EViewType::VolumetricRate, min_volumetric_speed, max_volumetric_speed);
    }
    wxBoxSizer* min_volumetric_speed_item = create_input_item(panel_velocity_volumetric, "min_volumetric_speed", _L("Min Volumetric Speed"),
                                                              wxT("mm\u00B3/s"), {double_min_checker});
    m_input_items["min_volumetric_speed"]->GetTextCtrl()->SetLabel(wxString::Format("%.2f", s_round(min_volumetric_speed, 2)));

    wxBoxSizer* max_volumetric_speed_item = create_input_item(panel_velocity_volumetric, "max_volumetric_speed", _L("Max Volumetric Speed"),
                                                              wxT("mm\u00B3/s"), {double_min_checker});
    ...
    wxStaticBitmap* helio_tid_copy = new wxStaticBitmap(last_tid_panel, wxID_ANY, create_scaled_bitmap("helio_copy", last_tid_panel, 24),
    ...
    /*helio wiki*/
    helio_wiki_link = new LinkLabel(this, _L("Click for more details"),
                                    wxGetApp().app_config->get("language") == "zh_CN" ? "https://wiki.helioadditive.com/zh/home" :
                                                                                        "https://wiki.helioadditive.com/en/home");
    ...
    /*confirm*/
    wxBoxSizer* button_sizer = new wxBoxSizer(wxHORIZONTAL);
    StateColor  btn_bg_green(std::pair<wxColour, int>(wxColour(175, 124, 255), StateColor::Enabled),
                ...
                wxCAPTION | wxCLOSE_BOX)
{
    ...
    helio_top_background->SetMinSize(wxSize(-1, FromDIP(70)));
    helio_top_background->SetMaxSize(wxSize(-1, FromDIP(70)));
    auto   helio_top_icon = new wxStaticBitmap(helio_top_background, wxID_ANY, create_scaled_bitmap("helio_icon", helio_top_background, 32),
    ...
    auto label_time_impro        = new Label(this, Label::Body_14,
                                             label_thrifty_time + " (" + label_original_time + " -> " + label_optimized_time + ")");
    ...
    auto tips = new Label(this, Label::Body_14,
                          L("Your gcode has been improved for the best possible print. To further improve your print please check out our "
                            "wiki for tips & tricks on what to do next."));
    ...
    auto rating_star1 = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("score_star_dark", this, 24), wxDefaultPosition,
                                           wxSize(FromDIP(24), FromDIP(24)), 0);
    auto rating_star2 = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("score_star_dark", this, 24), wxDefaultPosition,
                                           wxSize(FromDIP(24), FromDIP(24)), 0);
    auto rating_star3 = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("score_star_dark", this, 24), wxDefaultPosition,
                                           wxSize(FromDIP(24), FromDIP(24)), 0);
    auto rating_star4 = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("score_star_dark", this, 24), wxDefaultPosition,
                                           wxSize(FromDIP(24), FromDIP(24)), 0);
    auto rating_star5 = new wxStaticBitmap(this, wxID_ANY, create_scaled_bitmap("score_star_dark", this, 24), wxDefaultPosition,
                                           wxSize(FromDIP(24), FromDIP(24)), 0);
    ...
    save_icon->Bind(wxEVT_LEFT_DOWN,
                    [this](wxMouseEvent& e) { wxPostEvent(wxGetApp().plater(), SimpleEvent(EVT_GLTOOLBAR_EXPORT_SLICED_FILE)); });
    ...
    m_button_confirm->SetCornerRadius(FromDIP(12));
    m_button_confirm->Bind(wxEVT_LEFT_DOWN, [this](auto& e) { EndModal(wxID_CLOSE); });
    m_button_confirm->Bind(wxEVT_ENTER_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_HAND); });
    m_button_confirm->Bind(wxEVT_LEAVE_WINDOW, [this](auto& e) { SetCursor(wxCURSOR_ARROW); });
