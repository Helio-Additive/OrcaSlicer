...
SCENARIO("Placeholder parser coFloatsOrPercents vector access", "[PlaceholderParser]") {
    ...
    // Note: outer_wall_speed is coFloat (scalar) and small_perimeter_speed is
    // coFloatOrPercent (scalar) in the config definition. Override them with vector
    // types at runtime to exercise the coFloatsOrPercents PlaceholderParser path.
    config.set_deserialize_strict({
        { "nozzle_diameter", "0.4,0.4,0.4,0.4" },
        { "pressure_advance", "1.5,2.0,3.0,4.0" }  // coFloats non-nullable
    });
    {
        auto* ows = new ConfigOptionFloatsNullable();
        ows->values = { 60.0, 70.0, 80.0, 90.0 };
        config.set_key_value("outer_wall_speed", ows);
    }
    ...
    {
        auto* sps = new ConfigOptionFloatsOrPercentsNullable();
        sps->values = {
        ...
        config.set_key_value("small_perimeter_speed", sps);
    }
