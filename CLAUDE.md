...
- **Merge-base graft**: upstream-sync PRs are squash-merged (required by the release branch's "verified signatures" rule — a squash yields one signed commit instead of hundreds of unsigned upstream ones). Squashing discards upstream ancestry, so before each merge the workflow ephemerally grafts the last-synced upstream commit via `git replace --graft` to keep the merge-base correct. Without it, conflicts balloon (a ~5-file delta once exploded to 7,876). The graft is local-only and removed before any push. **Always squash-merge upstream-sync PRs, never a merge commit.** See `HELIO_INTEGRATION.md` → "Upstream Sync: squash merges & the merge-base graft".
- **`upstream-commit:` trailer — the baseline record**: every sync commit ends with `upstream-commit: <sha>` plus `upstream-sync-mode: tag|main`, written by both the clean-merge squash step and step 4 of the auto-created conflict issue's instructions. Both must sit in the **same** `-m` (git reads trailers only from the last paragraph, and each `-m` starts a new one). The graft reads the newest record **matching its own mode** on the release branch's first-parent line and prefers it over everything else — mode-scoped because tag and main syncs advance the branch along different upstream lines, so the newest record overall can be the wrong baseline. It reads them with a full-message scan rather than `%(trailers:…)`, because GitHub appends `Co-authored-by:` in its own paragraph at squash-merge time, which would orphan a strict trailer read. `git log --grep` is only a candidate prefilter — `--all-match` does not require the two lines to belong to the same record, so each candidate is re-scanned for an `upstream-commit:` line **immediately followed** by the matching mode line. It exists because the old derivation (`version.inc`, or the `helio-last-synced-main` tracking tag for `main` syncs) resolves through a **mutable** ref: if upstream retags a release from A to B after we synced A, the next sync accepts B as the baseline and merges only B..C, so everything in A..B is silently never merged — no conflict, no warning, a sync PR that looks clean. If the recorded sha does not resolve, the graft is **skipped** rather than falling back to a tag guess (a wrong graft under-merges silently; no graft only inflates the conflict count, which is loud and fixable). Commits with no trailer still use the legacy derivation. **When resolving a sync by hand, keep the trailer in the squash commit, and leave it in place when squash-merging the PR** — the release-branch commit is the only durable copy; the conflict issue's body is overwritten by the next run for the same tag.
