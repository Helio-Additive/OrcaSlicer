...
- Split into two jobs. `inventory` holds no credentials and runs on all events, including `pull_request`. `secret-presence` is excluded from `pull_request` and supplies W3 with secret **names** read from the Actions secrets REST API (which never returns values) using the optional `WORKFLOW_INVENTORY_TOKEN`; when that token is unset the job skips and W3 is not evaluated. Never use `toJSON(secrets)` here — it puts every secret name and value in the environment of a job running checked-out code
- Dependency extraction only scans Actions expression contexts (`${{ }}` spans and `if:` values), and matches both `secrets.NAME` and `secrets['NAME']`. Bracket form was previously missed, which is a false negative in the dangerous direction; scanning raw file text instead of expressions produced the opposite problem (a `/tmp/secrets.json` path read as a secret named `json`). Dynamic subscripts (`secrets[matrix.x]`) are reported rather than silently ignored
