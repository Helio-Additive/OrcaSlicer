...
- **`version.inc` sets an expectation; it never skips a run on its own.** It is a version string, and the release it names can stop describing the tree without the file changing (upstream force-retagging a release we already synced, or a local version bump landing before the content). Skipping there would silently drop upstream work — far worse than #107's empty PR. Two invariants protect this: every write of the tracking tag must be content-backed (that is why the *tag* may skip a run but `version.inc` may not), and the merge-base graft requires a **strict** ancestor — grafting a `version.inc` that names the target would make the merge a no-op by construction and reduce the tree comparison to a rubber stamp
